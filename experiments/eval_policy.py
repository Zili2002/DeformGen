...
def _save_final_soft_state(env, episode_dir: Path) -> None:
    """Persist final-frame soft-body state for offline success evaluation."""
    final_dir = episode_dir / "final_state"
    final_dir.mkdir(parents=True, exist_ok=True)

    state = env.unwrapped.get_state()
    with open(final_dir / "state.pkl", "wb") as f:
        pkl.dump(state, f)

    renderer_state = env.unwrapped.renderer.get_state()
    points_world = renderer_state["x"].detach().cpu().numpy()
    np.save(final_dir / "state.npy", {
        "points": points_world,
        "points_world": points_world,
        "num_object_points": int(points_world.shape[0]),
        "metadata": {"frame": "world", "pose_applied": True},
    })


def _load_deformed_state_paths(cfg):
    """Load optional per-episode deformed state paths from config."""
    deformed_state_paths = None
    deformed_states_file = cfg.get("deformed_states_file", None)
    deformed_states_dir = cfg.get("deformed_states_dir", None)
    deformed_states_pattern = cfg.get("deformed_states_pattern", "*.npy")
    ...
    if deformed_states_file is not None:
        ...
        deformed_state_paths = [str(deformed_state_path)]

    if deformed_state_paths is None:
        return None

    for p in deformed_state_paths:
        if not Path(p).exists():
            raise FileNotFoundError(f"Deformed state not found: {p}")
    return deformed_state_paths
...
def main(cfg):
    ...
    deformed_state_paths = _load_deformed_state_paths(cfg)
    if deformed_state_paths is not None:
        if len(deformed_state_paths) == 1:
            print(f"Using fixed deformed state for one episode: {deformed_state_paths[0]}")
            cfg.policy.n_episodes = 1
        else:
            print(f"Using per-episode deformed states: {len(deformed_state_paths)} files")
            cfg.policy.n_episodes = len(deformed_state_paths)
    ...
    randomize_reset = deformed_state_paths is None
    if not randomize_reset:
        print("Disabling renderer randomization because fixed/per-episode deformed states are provided.")

    for episode_id in range(cfg.policy.n_episodes):

        # Randomize only when eval is not driven by explicit deformed states.
        env = gym.make(cfg.env_name, max_episode_steps=frame_rate * duration + 30, cfg=cfg, 
            obs_mode=cfg.obs_mode, exp_root=cfg.exp_root, randomize=randomize_reset)
        episode_deformed_state = None
        if deformed_state_paths is not None:
            if len(deformed_state_paths) == 1:
                episode_deformed_state = deformed_state_paths[0]
            else:
                episode_deformed_state = deformed_state_paths[episode_id]

        if episode_deformed_state is None:
            obs, reset_info = env.reset(seed=episode_id)
        else:
            obs, reset_info = env.reset(seed=episode_id, deformed_state_path=episode_deformed_state)
        ...
        if episode_deformed_state is not None:
            with open(f'{out_path}/{run_name}/episode_{episode_id:04d}/deformed_state_path.txt', 'w') as f:
                f.write(f"{episode_deformed_state}\n")
        ...
        episode_dir = Path(f'{out_path}/{run_name}/episode_{episode_id:04d}')
        _save_final_soft_state(env, episode_dir)
