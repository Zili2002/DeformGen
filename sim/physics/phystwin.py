...
class SpringMassDynamicsModule:
    def __init__(
        ...
        if phystwin_cfg.collide_fric_override is not None:
            collide_fric_override = float(phystwin_cfg.collide_fric_override)
        else:
            override_by_case = phystwin_cfg.collide_fric_override_by_case
            if override_by_case is not None:
                case_prefix = case_name.split("_", 1)[0]
                if case_prefix in override_by_case and override_by_case[case_prefix] is not None:
                    collide_fric_override = float(override_by_case[case_prefix])
        if collide_fric_override is not None:
            collide_fric = torch.tensor(
                [collide_fric_override], dtype=torch.float32, device=self.device
            )
            phystwin_cfg.collide_fric = collide_fric_override
