...
def set_ik_log_path(path):
    """Enable IK logging to a CSV file; pass None to disable."""
    global _IK_LOG_PATH
    if path is None:
        _IK_LOG_PATH = None
        return
    ...
    _IK_LOG_PATH = str(log_path)
    with open(_IK_LOG_PATH, 'w') as f:
        f.write("frame,pose_diff,rot_diff\n")


def set_ik_log_frame(frame_idx):
    """Set the current frame index for IK logging."""
    global _IK_LOG_FRAME
    _IK_LOG_FRAME = frame_idx
...
class KinHelper():
    ...
    def compute_ik_sapien(self, initial_qpos, cartesian, verbose=False):
        ...
        if _IK_LOG_PATH is not None:
            frame = _IK_LOG_FRAME if _IK_LOG_FRAME is not None else -1
            with open(_IK_LOG_PATH, 'a') as f:
                f.write(f"{frame},{pose_diff:.8f},{rot_diff:.8f}\n")
