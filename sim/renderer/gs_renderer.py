...
class GSRenderer:
    ...
    def _render_camera(self, render_data, w, h, k, w2c, near, far, bg):
        max_sh_degrees = int(np.sqrt(render_data['shs'].shape[1]) - 1)
        if self.cfg.gs.use_shs:
            cam = setup_camera(w, h, k, w2c, near, far, bg,
                    z_threshold=0.0, sh_degree=max_sh_degrees, device=self.device)
            im, _, depth, = GaussianRasterizer(raster_settings=cam)(**render_data)
        else:
            cam = setup_camera(w, h, k, w2c, near, far, bg,
                    z_threshold=0.0, sh_degree=0, device=self.device)
    ...
    def _compose_rgba(self, im_black, im_white):
        alpha = 1.0 - torch.mean(im_white - im_black, dim=0, keepdim=True)
        alpha = torch.clamp(alpha, 0.0, 1.0)
        rgb = torch.zeros_like(im_black)
        mask = alpha > 1e-6
        rgb[mask.expand_as(rgb)] = (im_black / torch.clamp(alpha, min=1e-6))[mask.expand_as(rgb)]
        rgb = torch.clamp(rgb, 0.0, 1.0)
        rgba = torch.cat([rgb, alpha], dim=0)
        return rgba

    @torch.no_grad
    def render_rgba(self, render_data=None, camera=None):
        im_black, depth = self.render(render_data=render_data, bg=[0.0, 0.0, 0.0], camera=camera)
        im_white, _ = self.render(render_data=render_data, bg=[1.0, 1.0, 1.0], camera=camera)
        rgba = self._compose_rgba(im_black, im_white)
        return rgba, depth
    ...
    def render_wrist(self, render_data=None, bg=[0.0, 0.0, 0.0], camera=None):
        ...
        return self._render_camera(
            ...
            k,
            w2c,
            self.metadata_wrist['near'],
            self.metadata_wrist['far'],
            bg,
        )

    @torch.no_grad
    def render_wrist_rgba(self, render_data=None, camera=None):
        im_black, depth = self.render_wrist(render_data=render_data, bg=[0.0, 0.0, 0.0], camera=camera)
        im_white, _ = self.render_wrist(render_data=render_data, bg=[1.0, 1.0, 1.0], camera=camera)
        rgba = self._compose_rgba(im_black, im_white)
        return rgba, depth

    def render_fixed_cameras(self, return_rgba=False):
        im_list, depth_list = [], []
        for camera in self.cameras:
            if return_rgba:
                im, depth = self.render_rgba(camera=camera)
            else:
    ...
    def render_wrist_cameras(self, return_rgba=False):
        im_list, depth_list = [], []
        for camera in self.wrist_cameras:
            if return_rgba:
                im, depth = self.render_wrist_rgba(camera=camera)
            else:
